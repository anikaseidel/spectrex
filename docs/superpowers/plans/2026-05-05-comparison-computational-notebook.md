# Computational Benchmarks Notebook Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `notebooks/comparison_computational.ipynb` — a pre-executed, developer-facing notebook benchmarking memory footprint and runtime of `JAXOperator` vs `SciPySparseOperator`, and solve time/convergence of `JAXProximalSolver` vs `SpectralSolver`, to justify Phase 2 architectural choices.

**Architecture:** Three sections: (1) memory footprint as a function of K — analytical model + measured bytes; (2) runtime benchmarks — build time and apply/adjoint time vs K with JAX JIT warm-up distinguished; (3) solve time vs convergence — residual norm vs wall-clock for both solvers on a fixed K=5 scene.

**Tech Stack:** Python 3.12+, `spectrex` (JAXOperator, JAXProximalSolver, SciPySparseOperator, SpectralSolver, EigenspectraBasis, InstrumentConfig, NoiseModel), `numpy`, `matplotlib`, `timeit`, `tracemalloc`, `uv run jupyter nbconvert`
...
| Action | Path |
|--------|------|
| Create | `notebooks/comparison_computational.ipynb` |
| Create | `docs/content/comparison_computational.ipynb` (symlink) |
| Modify | `docs/index.rst` (add new "Internals" toctree section) |
...
- `SciPySparseOperator.build(config, basis, image_shape)` — builds full-image sparse matrix; no source positions passed.
...
- `JAXOperator` internal arrays: `._trace_indices` (JAX array, int32, shape `(K, n_orders, n_lambda)`), `._weights` (JAX array, float32, shape `(n_orders, n_lambda, M)`).
- `SciPySparseOperator` internal matrix: `._matrix` (scipy CSR sparse matrix). Its memory: `._matrix.data.nbytes + ._matrix.indices.nbytes + ._matrix.indptr.nbytes`.
- `JAXOperator` memory: `np.asarray(op._trace_indices).nbytes + np.asarray(op._weights).nbytes`.
- `SpectralSolver(op, noise_model=nm, regularisation=1e-2).solve(image_flat, support_mask=mask)`.
- `JAXProximalSolver(jax_op, noise_model=nm, lam=0.05, n_iter=200).solve(image_flat)`.
- Testdata: `REPO / 'testdata'` where `REPO = Path.cwd().parent`.
- Correct conf path: `TESTDATA / 'Config Files' / 'GR150R.F150W.220725.conf'`
- Correct wavelength range: `TESTDATA / 'jwst_niriss_wavelengthrange_0002.asdf'`
- Correct sensitivity dir: `TESTDATA / 'SenseConfig' / 'wfss-grism-configuration'`

---

## Task 1: Scaffold the notebook and instrument setup

**Files:**
- Create: `notebooks/comparison_computational.ipynb`
- Create: `notebooks/_build_comparison_computational.py`

- [ ] **Step 1: Create the build script scaffold**
...
```bash
cd notebooks && uv run python _build_comparison_computational.py
```

Expected: `Written: .../notebooks/comparison_computational.ipynb`

- [ ] **Step 3: Commit**

```bash
git add notebooks/comparison_computational.ipynb notebooks/_build_comparison_computational.py
git commit -m "chore: scaffold comparison_computational notebook"
```

---

## Task 2: Section 1 — Memory footprint

**Files:**
- Modify: `notebooks/_build_comparison_computational.py`
- Modify: `notebooks/comparison_computational.ipynb`

- [ ] **Step 1: Add analytical memory model and measurement cells**

Append to `_build_comparison_computational.py` (before `nb = new_notebook(cells=cells)` line):
...
```bash
git add notebooks/comparison_computational.ipynb notebooks/_build_comparison_computational.py
git commit -m "feat: add memory footprint section to computational notebook"
```

---

## Task 3: Section 2 — Runtime benchmarks

**Files:**
- Modify: `notebooks/_build_comparison_computational.py`
- Modify: `notebooks/comparison_computational.ipynb`

- [ ] **Step 1: Add runtime benchmark cells**

Append to `_build_comparison_computational.py` (before `nb = new_notebook(cells=cells)`):
...
```bash
git add notebooks/comparison_computational.ipynb notebooks/_build_comparison_computational.py
git commit -m "feat: add runtime benchmark section to computational notebook"
```

---

## Task 4: Section 3 — Solve time and convergence

**Files:**
- Modify: `notebooks/_build_comparison_computational.py`
- Modify: `notebooks/comparison_computational.ipynb`

- [ ] **Step 1: Add convergence cells**
...
cells.append(code(
    "# Run LSQR in chunks of 10 iterations, recording residual and wall-clock time\n"
    "from spectrex.operator import SciPySparseOperator as _SpOp\n\n"
...
cells.append(code(
    ...
    "# We run FISTA in 1-iteration chunks to record convergence\n"
    "import jax.numpy as jnp\n"
    "from spectrex.jax_solver import group_soft_threshold, power_iteration\n\n"
    "N_FISTA_ITER = 200\n"
    "LAM = 0.05\n\n"
    "L = power_iteration(jax_op_conv, w.reshape(IMAGE_SHAPE), N_PIX, n_iter=30)\n"
    "step = 1.0 / L\n\n"
...
```bash
git add notebooks/comparison_computational.ipynb notebooks/_build_comparison_computational.py
git commit -m "feat: add convergence comparison section to computational notebook"
```

---

## Task 5: Execute the notebook

**Files:**
- Modify: `notebooks/comparison_computational.ipynb` (adds outputs)
...
cd notebooks && uv run jupyter nbconvert \
    ...
    --inplace \
    comparison_computational.ipynb
```

Expected: exits 0; all code cells have outputs.

If a cell fails (e.g., JAX import error or attribute not found), read the error in the output cell, fix the build script, regenerate, and retry.
...
git commit -m "feat: execute comparison_computational notebook with outputs"
```

---

## Task 6: Symlink, toctree (new Internals section), and final commit
...
```bash
cd docs/content && ln -s ../../notebooks/comparison_computational.ipynb comparison_computational.ipynb
```

- [ ] **Step 2: Add a new "Internals" toctree to `docs/index.rst`**

The current `docs/index.rst` ends with the Examples toctree. Append a new toctree block after it:
...
```bash
rm notebooks/_build_comparison_computational.py
```

- [ ] **Step 4: Final commit**

```bash
git add docs/content/comparison_computational.ipynb docs/index.rst
git rm notebooks/_build_comparison_computational.py
git commit -m "feat: comparison_computational notebook — symlink and Internals toctree"
```
...
- Analytical + measured memory comparison (JAXOperator vs SciPySparseOperator vs K)
- Build time and apply/adjoint runtime vs K with JAX JIT cold vs warm
- Residual norm convergence vs wall-clock for LSQR and FISTA (K=5 sources)
- Summary table and "when to use which" guidance for developers
